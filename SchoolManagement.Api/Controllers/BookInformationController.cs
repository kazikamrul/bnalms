...
public class BookInformationController : ControllerBase
{
    ...
    public async Task<ActionResult> Put([FromForm] CreateBookInformationDto BookInformation)
    {
        var command = new UpdateBookInformationCommand { CreateBookInformationDto = BookInformation };
        var response = await _mediator.Send(command);
