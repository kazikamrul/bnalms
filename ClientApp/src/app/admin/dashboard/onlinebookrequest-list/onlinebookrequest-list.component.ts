...
@Component({
  selector: 'app-onlinebookrequest-list',
  templateUrl: './onlinebookrequest-list.component.html',
  styleUrls: ['./onlinebookrequest-list.component.sass']
})
export class OnlineBookRequestListComponent implements OnInit {
  masterData = MasterData;
  userRole = Role;
  isLoading = false;
  onlineBookRequest:any;
  showHideDiv:any;
  ...
  ngOnInit() {
    ...
    this.getOnlineBookRequestListByBookInfo(id);
  }
  backClicked() {
    this._location.back();
  }
  ...
  getOnlineBookRequestListByBookInfo(id) {
    this.DashboardService.getOnlineBookRequestListByBookInfo(id).subscribe(response => {           
      this.onlineBookRequest=response;
      console.log(this.onlineBookRequest);
    })
  }
