...
@Component({
  selector: 'app-new-bookinformationdamaged',
  templateUrl: './new-bookinformationdamaged.component.html',
  styleUrls: ['./new-bookinformationdamaged.component.sass']
})
export class NewBookInformationDamageComponent implements OnInit {
      ...
      this.BookInformationService.find(+id).subscribe(
        res => {
          this.BookInformationForm.patchValue({          
            ...
            authorDamageStatus:res.authorDamageStatus,
            reason:res.reason,
...
GetDepartmentNameById(baseNameId){    
  ...
  onSubmit() {
   ...
   this.BookInformationForm.get('mergeId').setValue(mergeId);
    ...
    this.BookInformationForm.get('damageDate').setValue((new Date(this.BookInformationForm.get('damageDate').value)).toUTCString());
