...
{
    public class GetBookIssueAndSubmissionListRequestHandler : IRequestHandler<GetBookIssueAndSubmissionListRequest, PagedResult<BookIssueAndSubmissionDto>>
    {
        ...
        public GetBookIssueAndSubmissionListRequestHandler(ISchoolManagementRepository<BookIssueAndSubmission> BookIssueAndSubmissionRepository, IMapper mapper)
        {
            _BookIssueAndSubmissionRepository = BookIssueAndSubmissionRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<BookIssueAndSubmissionDto>> Handle(GetBookIssueAndSubmissionListRequest request, CancellationToken cancellationToken)
        {
            ...
            IQueryable<BookIssueAndSubmission> BookIssueAndSubmissions = _BookIssueAndSubmissionRepository.FilterWithInclude(x => ((x.BookInformation.MergeId.Contains(request.QueryParams.SearchText) || String.IsNullOrEmpty(request.QueryParams.SearchText)) && x.ReturnStatus == 0 && x.BorrowerDamageStatus == 0 && x.BaseSchoolNameId == request.BaseSchoolNameId), "MemberInfo","BookInformation.BookTitleInfo", "BookInformation");
