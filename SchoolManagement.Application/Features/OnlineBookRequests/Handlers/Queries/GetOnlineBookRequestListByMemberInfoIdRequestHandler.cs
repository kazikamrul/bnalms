...
{
    public class GetOnlineBookRequestListByMemberInfoIdRequestHandler : IRequestHandler<GetOnlineBookRequestListByMemberInfoIdRequest, PagedResult<OnlineBookRequestDto>>
    {
        ...
        public GetOnlineBookRequestListByMemberInfoIdRequestHandler(ISchoolManagementRepository<OnlineBookRequest> OnlineBookRequestRepository, IMapper mapper)
        {
            _OnlineBookRequestRepository = OnlineBookRequestRepository;
            _mapper = mapper;
        }

        public async Task<PagedResult<OnlineBookRequestDto>> Handle(GetOnlineBookRequestListByMemberInfoIdRequest request, CancellationToken cancellationToken)
        {
            ...
            OnlineBookRequests = OnlineBookRequests.OrderByDescending(x => x.OnlineBookRequestId).Skip((request.QueryParams.PageNumber - 1) * request.QueryParams.PageSize).Take(request.QueryParams.PageSize).Where(x=>x.MemberInfoId ==request.MemberInfoId);
